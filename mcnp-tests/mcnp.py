  ...
  #                       1234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890
    ...
    def read_header(self):
        ...
        if 'mcnpx' in self.kod:
            ...
            print '      nps:',self.np1  
            print '    track:',self.nrss 
            print '   (6/11):',self.ncrd 
            print ' surfaces:',self.njsw 
            print 'hst to ss:',self.niss 
            ...
            print 'version    id:-'+ self.ver
            self.ver    = header.get_string( 9)[0]  # code version identifier
            print 'version  date:-'+ self.ver
            self.loddat = header.get_string(19)[0]  # code version date
            print "   start time:-"+ self.loddat
            self.idtm   = header.get_string(19)[0] # current date and time
            print "    stop time:-"+ self.idtm  
            self.probid = header.get_string(80)[0] # problem identification string
            print " title string:-"+ self.probid
            self.knod   = header.get_int()[0]      # dump number
            print self.knod  
            ...
            # interpret table lengths
            self.np1  = tablelengths.get_int()[0]  # number of histories used to generate source
            print '( np1)      nps:',self.np1  
            self.notsure0 = tablelengths.get_int()[0]  # number of values in surface source record
            print '( ns0)not sure0:',self.notsure0
            self.nrss = tablelengths.get_int()[0]  # number of tracks written to surface source
            print '(nrss)    track:',self.nrss 
            self.notsure1 = tablelengths.get_int()[0]   # number of surfaces
            print '( ns1)not sure1:',self.notsure1
            self.ncrd = tablelengths.get_int()[0]  # number of histories written to surface source
            print '(ncrd)   (6/11):',self.ncrd 
            self.njsw = tablelengths.get_int()[0]   # number of surfaces
            print '(njsw) surfaces?', self.njsw# number of histories written to surface source
            self.niss=tablelengths.get_int()[0]  # number of histories written to surface source
            print '(niss)hst to ss:',self.niss 
            self.notsure2 = tablelengths.get_int()[0]   # number of surfaces
            print '( ns2)not sure2:',self.notsure2

        if self.np1 < 0:
            # read table 2 record; more size info
            tablelengths = self.get_fortran_record()
            print tablelengths, 'should be 20' # should be 80
            ...
            print self.mipts
            print self.kjaq
            print tablelengths.get_int()[0]  # macrobody facet flag
            print tablelengths.get_int()[0]  # macrobody facet flag

	    print tablelengths.pos
            
            self.table2extra=[]
            while tablelengths.numBytes > tablelengths.pos:
                self.table2extra += tablelengths.get_int()
                print self.table2extra, tablelengths.numBytes, tablelengths.pos
            ...
            for j in range(self.njsw,self.njsw+self.niwr):
                extra=self.get_fortran_record()
                print extra
                print "Extra info in header not handled:", j

            # read summary table record
            summaryInfo = self.get_fortran_record()
            print 'summaryInfo:', summaryInfo
            print (2+4*self.mipts)*(self.njsw+self.niwr)+1
...
if __name__ == "__main__":
    SurfSrc('wssa.mcnpx').read_header()
    ss=SurfSrc('wssa.mcnp6')
    ss.read_header()
    ss.read_tracklist()
    print ss.print_tracklist()
    print 'here i am alone'
    #print ss.get_fortran_record()
